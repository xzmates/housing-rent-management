...
- On Windows, source files in this project are UTF-8 without BOM. Do **not** treat mojibake shown in the terminal as real file content. When reading source, config, or docs through PowerShell, always specify UTF-8 explicitly, for example `Get-Content -Encoding utf8`; do not use unqualified `Get-Content`, `cat`, `type`, or `Select-String`. Before editing a file that contains Chinese text, re-read it as UTF-8 and confirm the real content. Do not replace, repair, or delete Chinese based on previous mojibake output. For source writes, avoid `Out-File` and default `Set-Content`; prefer patch edits that preserve UTF-8 without BOM. If UTF-8 reading cannot be guaranteed, stop and report the issue.
