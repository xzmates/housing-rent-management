/*
 * 2026-07-31 全量自然语言重跑：以 120s 超时 + 失败重试补齐完整 finalText，
 * 输出到独立文件，不覆盖历史 raw-results。全程只读，不触碰任何写型 Skill。
 */
...
const sessionPrefix = 'AI_NL31_20260731'
...
const cases = [
  ...
  ['A12', '查2026年3月1日到2026年5月31日的入住名单。'], ['A13', 'NL测试欠费租客现在还有没付完的钱吗？'], ['A14', 'NL测试欠费租客那笔账单应付多少，已经付了多少，还剩多少？'],
  ['A15', 'NL测试欠费租客哪些账单只是部分付款？'], ['A16', 'NL测试欠费租客总欠款是多少？'], ['M01', '今天谁退租了？'], ['M02', '昨天有退租记录吗？'],
  ...
  ['D01', '现在谁欠钱？'], ['D02', '现在有哪些未缴账单？'], ['D03', '现在一共有多少欠费？'], ['D04', '谁还没交房租？'], ['D05', '哪些租客有未结清费用？'],
...
function looksTruncated(finalText) {
  ...
  return /[:：,，、]$/.test(t) || /^(?:目前|当前|今天|今年|共|合计)[^。！？]*$/.test(t)
}
...
async function runCase([id, question], index) {
  ...
  if (bad) {
    console.log(`[retry] ${id} attempt1 bad(calls=${result.calls?.length}, trunc=${looksTruncated(result.finalText)}) -> rerun`)
    result = await runOne([id, question], index, 2)
  }
  return result
}
...
async function main() {
  ...
  for (let index = 0; index < cases.length; index += 1) {
    const item = await runCase(cases[index], index)
    results.push(item)
    fs.writeFileSync(output, JSON.stringify({ generatedAt: new Date().toISOString(), total: cases.length, completed: results.length, results }, null, 2))
    const flags = []
    if (item.partial) flags.push('partial')
    if (!item.finalText) flags.push('NO_TEXT')
    if (looksTruncated(item.finalText)) flags.push('TRUNC')
    if (!item.calls?.length) flags.push('NO_TOOL')
    if (item.results?.some(r => r.isError)) flags.push('ERR')
    console.log(`[${index + 1}/${cases.length}] ${item.id} ${flags.join(' ') || 'OK'} calls=${(item.calls || []).map(c => c.name).join(',')}`)
  }
