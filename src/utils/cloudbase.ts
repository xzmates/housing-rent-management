import cloudbase from "@cloudbase/js-sdk";

// 云开发环境ID，使用时请替换为您的环境ID
export const ENV_ID = import.meta.env.VITE_ENV_ID || "housing-rent-management-401848f6";

// 检查环境ID是否已配置
export const isValidEnvId = ENV_ID && ENV_ID !== "your-env-id";

// 客户端Publishable Key, 可前往https://tcb.cloud.tencent.com/dev?envId={env}#/env/apikey获取
const PUBLISHABLE_KEY =  import.meta.env.VITE_PUBLISHABLE_KEY || "";

/**
 * 初始化云开发实例
 * @param {Object} config - 初始化配置
 * @param {string} config.env - 环境ID，默认使用ENV_ID
 * @param {number} config.timeout - 超时时间，默认15000ms
 * @param {number} config.accessKey - 客户端Publishable Key，默认使用PUBLISHABLE_KEY
 */
export const init = (config: { env?: string; timeout?: number; accessKey?: string; } = {}) => {
  const appConfig = {
    env: config.env || ENV_ID,
    timeout: config.timeout || 15000,
    accessKey: config.accessKey || PUBLISHABLE_KEY,
    auth: { detectSessionInUrl: true },
  };

  if (!appConfig.accessKey) {
    console.warn("客户端 Publishable Key 未配置");
  }

  return cloudbase.init(appConfig);
};

/**
 * 默认的云开发实例
 */
export const app = init();

/**
 * 检查环境配置是否有效
 */
export const checkEnvironment = () => {
  if (!isValidEnvId) {
    const message =
      "❌ 云开发环境ID未配置\n\n请按以下步骤配置：\n1. 打开 src/utils/cloudbase.js 文件\n2. 将 ENV_ID 变量的值替换为您的云开发环境ID\n3. 保存文件并刷新页面\n\n获取环境ID：https://console.cloud.tencent.com/tcb";
    console.error(message);
    return false;
  }
  return true;
};

interface LoginResult {
  isLoggedIn: boolean;
  user: {
    uid: string;
    isAnonymous: boolean;
    email?: string;
    phone?: string;
    username?: string;
    displayName?: string;
  };
  session?: any;
}

/**
 * 检查用户登录态
 * @returns {Promise} 登录状态
 */
export const checkLogin = async (): Promise<LoginResult> => {
  // 检查环境配置
  if (!checkEnvironment()) {
    throw new Error("环境ID未配置");
  }

  const auth = app.auth;

  try {
    // 先尝试获取claims来判断用户类型
    const { data: claimsData } = await auth.getClaims();

    if (claimsData.claims?.sub === "anon") {
      console.log("匿名用户 - 使用Publishable Key访问");

      return {
        isLoggedIn: true,  // 匿名用户视为已登录
        user: {
          uid: "anon:" + Date.now(),  // 生成唯一匿名ID
          isAnonymous: true,
        },
      };
    }

    // 尝试获取session来判断正式登录状态
    const { data: sessionData } = await auth.getSession();

    if (sessionData.session) {
      console.log("用户已登录 - 正式用户");

      const user = sessionData.session.user || {};
      // 安全地访问属性，避免类型错误
      const userObj = user as any;
      return {
        isLoggedIn: true,
        user: {
          uid: userObj.uid || userObj.id || userObj._id || '',
          isAnonymous: userObj.is_anonymous || userObj.isAnonymous || false,
          email: userObj.email,
          phone: userObj.phone,
          username: userObj.username,
          displayName: userObj.displayName,
        },
        session: sessionData.session,
      };
    }

    // 既不是匿名用户也没有session，返回未登录
    return {
      isLoggedIn: false,
      user: {
        uid: '',
        isAnonymous: false,
      },
    };

  } catch (error: any) {
    console.warn("登录检查异常:", error.message || error);

    // 如果出现错误，尝试简化检查
    try {
      const { data: claimsData } = await auth.getClaims();
      if (claimsData.claims?.sub === "anon") {
        return {
          isLoggedIn: true,
          user: {
            uid: "anon-error-" + Date.now(),
            isAnonymous: true,
          },
        };
      }
    } catch (innerError) {
      console.warn("内部检查失败:", innerError);
    }

    // 默认返回未登录
    return {
      isLoggedIn: false,
      user: {
        uid: '',
        isAnonymous: false,
      },
    };
  }
};

/**
 * 退出登录
 */
export const logout = async (): Promise<{ success: boolean; message: string }> => {
  const auth = app.auth;

  try {
    await auth.signOut();
    return { success: true, message: "已成功退出登录" };
  } catch (error) {
    console.error("退出登录失败:", error);
    throw error;
  }
};

// 默认导出
export default {
  init,
  app,
  checkLogin,
  logout,
  checkEnvironment,
  isValidEnvId,
};
